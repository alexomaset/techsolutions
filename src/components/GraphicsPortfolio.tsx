...
const OrbitalGraphicsPortfolio = () => {
  ...
  // Get responsive orbit radius based on screen size
  const getOrbitRadius = () => {
    if (typeof window !== 'undefined') {
      // Adjust radius based on viewport width
      if (window.innerWidth < 640) return 120; // Small mobile
      if (window.innerWidth < 768) return 160; // Mobile
      if (window.innerWidth < 1024) return 200; // Tablet
    }
    return 240; // Desktop default
  };
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setOrbitItems(prev => {
        ...
        return prev.map(item => ({
          ...item,
          x: Math.cos(item.angle) * (radius / 240),
          y: Math.sin(item.angle) * (radius / 240),
        }));
      });
    };
  ...
  return (
    <section className="py-20 bg-gray-100 text-gray-800 overflow-hidden relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* Orbital Display */}
        <div className="relative h-[500px] sm:h-[600px] w-full" ref={orbitRef}>
          {/* Orbit lines */}
          <motion.div 
          ...
          {orbitItems.map((item) => {
            ...
            const orbitRadius = getOrbitRadius(); // Responsive radius
            ...
            return (
              ...
              >
                ...
                >
                  <div className="relative w-32 h-28 sm:w-48 sm:h-40 rounded-lg overflow-hidden border border-gray-300 shadow-lg">
                    <Image
                      src={item.src}
                      alt={item.title}
                      fill
                    ...
                    <div className="absolute inset-0 bg-white/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <div className="text-gray-800 text-xs sm:text-sm font-medium text-center p-1">
                        {item.title}
                      </div>
                    </div>
                  </div>
        ...
        <div className="mt-8 sm:mt-12 flex flex-wrap justify-center gap-2 sm:gap-4">
          ...
          <motion.button
            className="px-3 py-2 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 bg-white text-gray-700 hover:bg-gray-200"
        ...
        >
          <motion.button
            className="px-6 py-3 sm:px-8 sm:py-4 bg-gradient-to-r from-blue-600 to-blue-400 text-white rounded-lg font-semibold hover:from-blue-500 hover:to-blue-300 transition-all duration-300 shadow-lg inline-flex items-center"
          ...
          >
            ...
            <AnimatePresence mode="wait">
              {graphics.find(item => item.id === selectedImage) && (
                <motion.div
                  key={selectedImage}
                  className="relative w-full max-w-5xl max-h-[80vh] bg-white rounded-lg overflow-hidden"
